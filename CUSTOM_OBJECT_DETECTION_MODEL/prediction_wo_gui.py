...
model = load_model(r"cnn8grps_rad1_model.h5")
...
hd2 = HandDetector(detectionCon=0.7)
...
while True:
    try:
        ...
        print(frame.shape)
        if hands:
            hands = hands[0]  # Get the list of hand dictionaries
            if isinstance(hands, list) and len(hands) > 0:
                for hand in hands:
                    if isinstance(hand, dict) and 'bbox' in hand:
                        x, y, w, h = hand['bbox']  # Assign values to x, y, w, h here
                        ...
                        if handz:
                            handz = handz[0]  # Get the list of hand dictionaries for the zoomed-in image
                            if isinstance(handz, list) and len(handz) > 0:
                                for hand in handz:
                                    if isinstance(hand, dict) and 'lmList' in hand:
                                        pts = hand['lmList']
                                        # x1, y1, w1, h1 = hand['bbox']
                                    else:
                                        print(f"Unexpected format for hand in handz: {hand}")
                                        continue
                            else:
                                print("No hands detected in zoomed-in image")
                        else:
                            print("No hands detected in zoomed-in image")
                    else:
                        print(f"Unexpected format for hand: {hand}")
                        print("hands[0]:", hand)  # Print the actual hand structure
                        continue
            else:
                print("No hands detected")

        # Now 'w' and 'h' are defined within the scope of the 'for' loop
        os = ((400 - w) // 2) - 15
        os1 = ((400 - h) // 2) - 15
        ...
        l = [[5, 2], [5, 3], [3, 5], [3, 6], [3, 0], [3, 2], [6, 4], [6, 1], [6, 2], [6, 6], [6, 7], [6, 0],
             [6, 5], [4, 1], [1, 0], [1, 1], [6, 3], [1, 6], [5, 6], [5, 1], [4, 5], [1, 4], [1, 5], [2, 0],
             [2, 6], [4, 6], [1, 0], [5, 7], [1, 6], [6, 1], [7, 6], [2, 5], [7, 1], [5, 4], [7, 0], [7, 5],
             [7, 2]]
        if pl in l:
        ...
        if pl in l:
            if pts[6][1] > pts[8][1] and pts[14][1] < pts[16][1] and pts[18][1] < pts[20][1] and \
                    pts[0][0] < pts[8][0] and pts[0][0] < pts[12][0] and pts[0][0] < pts[16][0] and \
                    pts[0][0] < pts[20][0]:
                ch1 = 3
                print("33333c")
        ...
        # con for [b][pqz]
        l = [[5, 0], [5, 1], [5, 4], [5, 5], [5, 6], [6, 1], [7, 6], [0, 2], [7, 1], [7, 4], [6, 6],
             [7, 2], [5, 0], [6, 3], [6, 4], [7, 5], [7, 2]]
        pl = [ch1, ch2]
        if pl in l:
            if (pts[6][1] > pts[8][1] and pts[10][1] > pts[12][1] and pts[14][1] > pts[16][1] and
                    pts[18][1] > pts[20][1]):
        ...
        # con for [i][pqz]
        l = [[5, 4], [5, 5], [5, 1], [0, 3], [0, 7], [5, 0], [0, 2], [6, 2], [7, 5], [7, 1], [7, 6],
             [7, 7]]
        pl = [ch1, ch2]
        if pl in l:
            if ((pts[6][1] < pts[8][1] and pts[10][1] < pts[12][1] and pts[14][1] < pts[16][1] and
        ...
        if pl in l:
            if not (
                    pts[0][0] + fg < pts[8][0] and pts[0][0] + fg < pts[12][0] and pts[0][0] + fg < pts[
                        16][0] and pts[
                        0][0] + fg < pts[20][0]
            ) and not (
                    pts[0][0] > pts[8][0] and pts[0][0] > pts[12][0] and pts[0][0] > pts[
                        16][0] and pts[
                        0][0] > pts[20][0]
